// Self-contained observability for EA agent (standalone Docker build — no @waas/runtime access)
...
export function initSentry(): void {
  ...
  if (!dsn) {
    console.warn("[Sentry] SENTRY_DSN not set — error tracking disabled (blockdrive-ea)");
    return;
  }
  ...
  try {
    ...
    console.log("[Sentry] Initialized (server=blockdrive-ea)");
  } catch (err) {
