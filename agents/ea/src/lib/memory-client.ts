/**
 * Redis Memory Client — EA standalone implementation
 *
 * Redis-backed memory system for the EA agent. Uses the same idx:memories
...
async function ensureIndex(redis: RedisClientType): Promise<void> {
  ...
  try {
    ...
    indexReady = true;
    console.log(`[EA Memory] Index created: ${INDEX_NAME}`);
  } catch (err) {
    indexFailCount++;
    console.error(`[EA Memory] Index creation failed (${indexFailCount}/${MAX_INDEX_RETRIES}):`, err);
    if (indexFailCount >= MAX_INDEX_RETRIES) {
      console.error("[EA Memory] CIRCUIT OPEN: Stopped retrying index creation.");
    }
  }
}
...
export async function addOrgMemory(
  ...
  const fields: Record<string, string | Buffer> = {
    text,
    agent_id: opts?.agentId ?? "blockdrive-ea",
...
export async function searchOrgMemories(
  ...
  try {
    ...
    console.error("[EA Memory] Search failed:", err);
    return [];
  }
}
...
export async function getSessionMemories(
  ...
  try {
    ...
    console.error("[EA Memory] Session search failed:", err);
    return [];
  }
}
...
export async function getAllOrgMemories(
  ...
  try {
    ...
    console.error("[EA Memory] GetAll failed:", err);
    return [];
  }
}
