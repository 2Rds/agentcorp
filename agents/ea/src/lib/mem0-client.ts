...
export async function addOrgMemory(
  ...
  const graphCategories = new Set([
    "contacts", "cross_department", "investor_relations", "hiring",
  ]);
...
/**
 * Search across all agents' memories (cross-namespace executive read).
 * EA has wildcard read access across all department namespaces.
 */
