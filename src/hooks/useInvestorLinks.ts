...
export function useInvestorLinks() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("link_views_realtime")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "link_views", filter: `organization_id=eq.${orgId}` },
        () => {
  ...
  // Compute analytics per link
  const analytics: LinkAnalytics[] = (linksQuery.data ?? []).map((link) => {
    const views = (viewsQuery.data ?? []).filter((v) => v.link_id === link.id);
    const totalViews = views.length;
    const uniqueEmails = new Set(views.map((v) => v.viewer_email).filter(Boolean));
    const uniqueViewers = uniqueEmails.size || totalViews;
    const avgDuration = totalViews > 0 ? views.reduce((s, v) => s + v.duration_seconds, 0) / totalViews : 0;
    const avgCompletion = totalViews > 0
      ? views.reduce((s, v) => (v.total_pages > 0 ? s + v.pages_viewed / v.total_pages : s), 0) / totalViews * 100
      : 0;
    const lastViewedAt = views.length > 0 ? views[0].started_at : null;
    return { link, views, totalViews, uniqueViewers, avgDuration, avgCompletion, lastViewedAt };
  });
