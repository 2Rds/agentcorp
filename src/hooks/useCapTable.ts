...
export function computeCapTableSummary(entries: CapTableEntry[]): CapTableSummary {
...
export function useCapTable(orgId: string | null) {
  ...
  const summary = useMemo<CapTableSummary>(() => computeCapTableSummary(query.data ?? []), [query.data]);
