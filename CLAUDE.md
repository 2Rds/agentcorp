...
- Claude Opus 4.6 (Anthropic direct) — Primary reasoning, tool orchestration, streaming chat, all customer-facing output
- Gemini 3 Flash (OpenRouter `gemini` alias) — Vision/OCR, internal orchestration, structured generation
- Grok 4.1 Fast (OpenRouter `grok-fast` alias) — X/Twitter data, classification
- Sonar Pro (Perplexity/OpenRouter `sonar` alias) — Live web search
